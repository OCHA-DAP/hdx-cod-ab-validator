...
const NUMERIC_TYPES = new Set(['INTEGER', 'INT', 'BIGINT', 'HUGEINT', 'SMALLINT', 'TINYINT', 'UBIGINT', 'DOUBLE', 'FLOAT', 'REAL', 'DECIMAL', 'NUMERIC']);
...
FROM data
    ...
    const badFormatSample = Array.from((row.bad_format_sample as Iterable<unknown>) ?? []).map(String);
    const alpha3Count = Number(row.alpha3_count);
    const delimitedCount = Number(row.delimited_count);
    const delimitedSample = Array.from((row.delimited_sample as Iterable<unknown>) ?? []).map(String);

...
- **Astro** with **Svelte 5 runes** (`$state`, `$derived`, `$bindable`)
...
- Initialized as singleton on first component mount (`src/lib/db/duckdb.svelte.ts`)
- Requires COOP/COEP headers (configured in `astro.config.mjs`) for `SharedArrayBuffer`
